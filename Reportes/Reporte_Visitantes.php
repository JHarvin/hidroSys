...
////Para los grado
        ...
        $pdf->SetFont('Arial', 'B', 12);
         $validar = mysqli_query($mysqli, "SELECT nombre, dui FROM `visitantes` GROUP BY tipo
ORDER BY
tipo ASC");
        
       if(mysqli_num_rows($validar)>0){ 
  $visitantes = mysqli_query($mysqli, "SELECT nombre, dui FROM `visitantes` GROUP BY tipo
ORDER BY
tipo ASC");
  $miCabecera = array('DUI', 'Nombre');
 
  while($row1 = mysqli_fetch_array($visitantes)) {
    $misDatos[] = array('dui' => $row1['dui'],'nombre' => $row1['nombre']
            );
