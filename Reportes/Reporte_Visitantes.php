...
tipo ASC");
  ...
  $visitantes = mysqli_query($mysqli, "SELECT
tipo,
dui,
nombre
FROM
...
ORDER BY
tipo ASC");
  $miCabecera = array('tipo', 'dui', 'nombre');
 
  while($row1 = mysqli_fetch_array($visitantes)) {
    $misDatos[] = array('tipo' => $row1['tipo'],'dui' => $row1['dui'],'nombre' => $row1['nombre']
            );
...
$pdf->Cell(70, 10, 'Dui: ' , 0, 0, 'C');
$pdf->Cell(70, 10, 'Nombre: ' , 0, 0, 'C');
$pdf->Ln(10);
$pdf->SetWidths(array(40,70,70));
   while ($row1 = mysqli_fetch_array($visitantes)) {
    $tipo=$row1['tipo'];
    $dui=$row1['dui'];
    $nombre=$row1['nombre'];
...
$pdf->Cell(40, 10, 'Tipo: ' .$tipo, 0, 0, 'C');
$pdf->Cell(70, 10, 'Dui: ' .$dui, 0, 0, 'C');
$pdf->Cell(70, 10, 'Nombre: ' .$nombre, 0, 0, 'C');*/
